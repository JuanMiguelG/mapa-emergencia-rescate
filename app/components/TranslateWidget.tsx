  ...
  return (
    <div ref={ref} className="relative">
      ...
      <button
        ...
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
        className="inline-flex min-h-10 shrink-0 items-center justify-center gap-1 rounded-full bg-slate-50 px-2.5 py-2 text-xs font-bold text-slate-800 shadow-sm transition hover:bg-slate-100 lg:min-h-9 lg:rounded-lg lg:border lg:border-slate-200 lg:bg-white lg:px-2 lg:py-1.5 lg:font-semibold lg:hover:border-slate-300 lg:hover:bg-slate-50 xl:px-2.5"
      >
        ...
        <span className="lg:hidden">{activeLang.toUpperCase()}</span>
      </button>
      {open && dropdown}
    </div>
  );
